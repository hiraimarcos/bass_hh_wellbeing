...
get_data_for_quarter <-function(year, quarter){
  ...
  df1$hh_id <- paste0(as.character(df1$UPA), as.character(df1$hh_number), as.character(df1$sampling_group))
...
write.csv(rbind(q1,q2), "data/pnad_q1q2_individual.csv")
...
q12 <- rbind(q1,q2)
write.csv(q12, "data/pnad_q1q2_individual_bothpresent.csv")


# get data aggregated by household:
summed <- q12 %>% get_dummies() %>% aggregate_sum()
firsts <- q12 %>% get_dummies() %>% aggregate_first(to_keep)
q12_hh <- tibble(merge_hh(firsts, summed, by.x=c("hh_id", "Trimestre", "Ano"), by.y=c("hh_id", "Trimestre", "Ano")))
...
write.csv(q13, "data/pnad_q1q3_individual_bothpresent.csv")


# Do same for first and second quarters of 2019:
...
write.csv(q12_19, "data/pnad_q1q2_2019_individual_bothpresent.csv")

# save data from first and second quarter of of 2019 and 2020 in one file
write.csv(rbind(q12, q12_19), "data/pnad_q1q2_2019_2020_individual.csv")
...
write.csv(df1920, "data/pnad_q1q2_2019_2020_individual.csv")
...
m19 <- merge(merge(merge_hh(s1_20, f1_20), merge_hh(s2_20, f2_20), by="hh_id"), merge_hh(s3_20, f3_20), by="hh_id")
m19 <- merge(merge(merge_hh(s1_19, f1_19), merge_hh(s2_19, f2_19), by="hh_id"), merge_hh(s3_19, f3_19), by="hh_id")
